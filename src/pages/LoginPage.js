...
export const LoginPage = () => {
    ...
    // const [selectedOption, setSelectedOption] = useState('login');
    // const [actionUrl, setActionUrl] = useState('/default-action');
   ...
   /* const [userData, setUserData] = useState({
    ...
    const handleLogin = () => {
        ...
        const credentials = {
          ...
          instructor: { username: 'instructor', password: '456' },
          pc: { username: 'programcoordinator', password: '789' },
          qa: { username: 'qaofficer', password: '123' },
          admin: { username: 'admin', password: '456' },
        };
    
        // Check if the entered credentials match any of the dummy credentials
    const userRole = Object.keys(credentials).find(
        (role) => username === credentials[role].username && password === credentials[role].password
      );
  
      if (userRole) {
        // Redirect to the respective page based on the matched role
        window.location.href = `/${userRole}`;
      } else {
        // Handle invalid credentials (you can add an error message here)
        console.log('Invalid credentials');
      }
    }
    

    // Make a GET request to a specific endpoint directly in the component
    ...
    //     //formRef.current.action = newActionUrl;
    ...
    /*const handleFormSubmit = (event) => {
        ...
        fetchData(event); // Call fetchData to make the GET request
    };*/
        ...
        <div>
            ...
            <div className="container_login" style={{ backgroundColor: '#f5f5f5' }}>
                ...
                <form className="login-form"  method="POST">
                    ...
                    {/* <select id="role" value={selectedOption} onChange={handleOptionChange}>
                        <option value="login">select role</option>
